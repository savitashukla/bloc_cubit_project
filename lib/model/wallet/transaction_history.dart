...
class TransactionWallet {
  ...
  List<TransactionList>? credit;
  List<TransactionList>? debit;
  Pagination? pagination;

  TransactionWallet({this.data, this.pagination});

  TransactionWallet.fromJson(Map<String, dynamic> json) {
    if (json['data'] != null) {
      data = <TransactionList>[];
      credit = <TransactionList>[];
      debit = <TransactionList>[];
      json['data'].forEach((v) {
        data!.add(new TransactionList.fromJson(v));
        if(v["mode"]=="debit")
        {
          debit!.add(new TransactionList.fromJson(v));
        }

        else if(v["mode"]=="credit")
        {
          credit!.add(new TransactionList.fromJson(v));
        }
...
class Amount {
  ...
  bool isBonuseType(){
    ...
    HelpMethod().customPrint("is_bonuse_type==>${is_bonuse_type}");
